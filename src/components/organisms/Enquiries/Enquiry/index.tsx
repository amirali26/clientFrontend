...
import REQUEST_CALLBACK from '../../../../mutations/enquiryMutations';
...
export const Enquiry: React.FC<Props> = ({
  ...
  const [requestCallback, { loading, data }] = useMutation(REQUEST_CALLBACK, {
    onCompleted: () => {
      sb.trigger('Solicitor notified to call you back', 'success');
    },
    onError: () => {
      sb.trigger('Error sending callback request to solicitor', 'error');
    },
  });
  return (
    <form className="flex spaceBetween column" style={{ height: '100%' }}>
      ...
      <div className="fullWidth  marginBottomMedium marginTopMedium">
      ...
      <div className="fullWidth marginBottom marginTop">
        ...
        >
          {
            loading || data ? 'Callback Requested' : 'Request Callback from solicitor'
          }
        </Button>
      </div>
    </form>
